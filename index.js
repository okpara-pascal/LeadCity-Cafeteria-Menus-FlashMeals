/* ─── Configuration ─────────────────────────────────────────────────────────
   TOKENS: Add or remove tokens here.
...
// per‑cafeteria last‑updated timestamps
const lastUpdated = {};
try {
...
function tryEnter() {
  ...
  // Check if token already used (only for non‑special tokens)
  ...
  // Enforce single‑use for normal tokens, but allow reuse for special tokens
...
document.getElementById('logout-btn').addEventListener('click', function () {
  ...
  localStorage.removeItem('selected_caf');   // ⬅️ clear cafeteria memory on logout
  ...
  updateModeBtn();
  // Re‑hide the sign‑out button until next special login
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (savedToken && VALID_TOKENS[savedToken]) {
    ...
    // If the token is special, we do NOT enforce single‑use again
    // (it’s already in use). If it’s normal, it was already marked as used.
    document.getElementById('gate').style.display = 'none';
    document.getElementById('app').style.display = 'flex';
    document.getElementById('logged-as').textContent = user.name;
    ...
    if (lastCaf && menus[lastCaf]) {
      selectedCaf = +lastCaf;
      renderCafGrid();          // re‑render to highlight the active card
      renderMenu(selectedCaf);  // show the cafeteria's menu
    }
  }
});
...
function selectCaf(id) {
  selectedCaf = id;
  localStorage.setItem('selected_caf', id);   // ⬅️ remember selected cafeteria
  renderCafGrid();
  renderMenu(id);
}
